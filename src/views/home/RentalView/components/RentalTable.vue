<template>
  ...
  >
    <template #header>
      <BaseTable.Header>
        <span class="w-1/5 px-2">Filme</span>
        <span class="w-1/5 px-2 text-center md:text-start">Cliente</span>
        <span class="w-1/5 px-2 text-right md:text-start">Data de locação</span>
        <span class="w-1/5 px-2 text-right md:text-start">Data de entrega</span>
        <span class="w-1/5 px-2 text-right md:text-start">Status</span>
      </BaseTable.Header>
    </template>
    ...
    <template #default="props: iRental">
      <BaseTable.Cell @click="$emit('selectRental', props)">
        ...
        <div class="w-1/5 px-2 text-center md:text-start">
          <span class="text-gray-700 text-base block">
            {{ props.clientName }}
          </span>
        </div>
        <div class="w-1/5 px-2 flex justify-end md:block">
